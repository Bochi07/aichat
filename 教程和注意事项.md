...
> ⚠️ **注意**：FastAPI 是 ASGI 应用，uWSGI 默认的 WSGI 模式无法直接运行。本项目推荐使用 Gunicorn（见 3.1 节）。如果你一定要用 uWSGI，需要安装 ASGI 插件（`pip install uwsgi[asgi]`）并使用 `--asgi` 模式，配置方式与当前 `uwsgi.ini` 不同，请参考 uWSGI 官方文档。

<details>
<summary>旧版 uWSGI 配置（仅供参考，不保证可用）</summary>
