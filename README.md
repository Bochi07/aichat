...
| `TRUSTED_PROXIES` | 否 | `127.0.0.1,::1` | 可信反向代理地址，逗号分隔。**仅当直连方在此列表中时才会采信 `X-Forwarded-For`**（用于获取真实客户端 IP 做限流与失败锁定）。若 nginx/WAF 与应用不在同一台机器，必须把它们的回源 IP 加进来，否则所有访客会被当成同一来源、互相挤占限流额度（启动日志会给出 `[WARN]` 提示） |
| `CLIENT_IP_HEADER` | 否 | 空 | 指定"客户端 IP 专用头"，如 `cf-connecting-ip`、`x-real-ip`。适用于云 WAF/CDN 回源 IP 不固定的场景（比维护 `TRUSTED_PROXIES` 更省事）。**⚠️ 仅当你的代理/WAF 会覆盖该头时才安全**；若它只原样透传，客户端就能自己伪造 |
...
密码直接以明文存入 SQLite 的 `users.password`，管理上可以直接查看和修改。
密码按 UTF-8 处理，**中文、日文、emoji、任意符号都可以作为密码**。

- 设置新密码时要求 8–128 个字符（调整 `main.py` 中的 `PASSWORD_MIN_LEN` 可改最小长度）；
- **早期版本写入的哈希密码不受影响**：那些账号依然能用原密码正常登录，并会在登录成功的那一刻
  自动写回明文（哈希不可逆，但登录时服务端正好拿到了明文），用户无感、**不需要重新设置密码**；
...
- **用户隔离** — 所有对话 / 消息 / 密钥接口都先按 `user_id` 校验归属，越权访问一律 404；动态响应统一禁止缓存，避免 WAF/CDN 按 URL 缓存导致跨用户串号（可随时用 `python tests/test_isolation.py` 回归验证）
- **访问限流** — 登录/注册/发送等接口按 IP/用户限流，超出返回 429（仅采信可信代理带来的 `X-Forwarded-For`）
- **登录失败锁定** — 同一"账号 + 来源 IP"连续失败 5 次锁定 15 分钟，计数落库、多进程有效，且无法被用来锁住别人
...
1. **密码明文存储** — 密码直接以明文存入 SQLite，这是本项目有意保留的特色（管理上可直接查看）。代价是数据库一旦泄露，任何人都能直接登录他人账号
2. **API Key 明文存储** — 各提供商的 API Key 以明文存入数据库（设置页需要完整回显，便于核对）
3. **内存 Token 黑名单** — JWT 注销机制使用内存存储，重启后失效（但登出同时会删除数据库中的会话记录，因此仍然有效）
4. **找回密码凭据较弱** — 仅凭"用户名 + 绑定的 QQ 号"，建议不要将本项目直接暴露到公网

这些设计是为了保持代码简洁、易于理解和修改。**若要用于生产环境，请自行补充密码与 API Key 的加密存储。** 建议不要将本项目直接暴露到公网。
